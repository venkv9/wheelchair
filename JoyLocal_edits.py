...
def nousb_joystick_read_thread():
    global joystick_x
    global joystick_y
    global rnet_threads_running
    
    while rnet_threads_running:
        #Range for jvalue: -32767 and +32767
        #changing x is for right(+) and left(-)
        #changing y is for back(+) and infront(-)
        jvalue_x=-15000
        jvalue_y= -32767
    
        try:
            joystick_x = 0x100 + int(jvalue_x * 100 / 128) >> 8 &0xFF
            joystick_y = 0x100 - int(jvalue_y * 100 / 128) >> 8 &0xFF
...
if __name__ == "__main__":
    ...
    #init usb joystick vars (not joy itself)
    joystick_x = 0
    joystick_y = 0


    nousb_joystick_read_thread = threading.Thread(
        target=nousb_joystick_read_thread,
        daemon=True)
    nousb_joystick_read_thread.start()
